...
def naiveSoftmaxLossAndGradient(centerWordVec, outsideWordIdx, outsideVectors,
    ...
    y_diff[outsideWordIdx] = y_hat[outsideWordIdx] - 1
    # dJ_dv_c (m x 1) = (m x W) @ (W x 1)
    gradCenterVec = outsideVectors.T @ y_diff
    # y @ v^T # (W x m) = (W x m) x (m x 1)
    gradOutsideVecs = np.outer(y_diff, centerWordVec)
    # ## END YOUR CODE
...
def negSamplingLossAndGradient(centerWordVec,
    ...
    z = sigmoid(u_o.T @ centerWordVec)  # (1, 1) = (1 x m) x (m x 1)
    #
    loss = -1.0 * np.log(z)
    gradCenterVec = (z - 1) * u_o  # (1 x 1) x (m x 1) = (m x 1)
    gradOutsideVecs[outsideWordIdx] = (z - 1) * centerWordVec  # W x 1

    #####
    # remove for loop by using matrix ops? U_neg = (m x K)
    # z = sigma(U_neg @ v_c.T[:, None]): (m x K) @ (1 x 1 x m) = (K x m)
    for k in range(1, K + 1):  # neg ids start from 1!?
        u_k = outsideVectors[indices[k]]  # (m x 1)  handle duplicates? k = 10
        z_neg = sigmoid(u_k.T @ centerWordVec)  # (1 x m) x (m x 1) = (1 x 1)
        loss -= np.log(1 - z_neg)  # (1 x 1)
        gradCenterVec += z_neg * u_k.T  # (1 x 1) x (1 x m)
        gradOutsideVecs[indices[
            k]] += z_neg * centerWordVec.T  # (1 x 1) x (1 x m) = (1 x m)
...
def skipgram(currentCenterWord,
    ...
    outsideWords -- list of no more than 2*windowSize strings, the outside words # noqa
    ...
    # ## YOUR CODE HERE
    v_c_id = word2Ind[currentCenterWord]
    v_c = centerWordVectors[v_c_id]
    for word in outsideWords:
        u_o_id = word2Ind[word]
        new_loss, new_grad_vc, new_grad_u_o = word2vecLossAndGradient(
            centerWordVec=v_c,
            outsideWordIdx=u_o_id,
            outsideVectors=outsideVectors,
            dataset=dataset)
        loss += new_loss
        gradCenterVecs[v_c_id] += new_grad_vc
        gradOutsideVectors += new_grad_u_o
    # ## END YOUR CODE
